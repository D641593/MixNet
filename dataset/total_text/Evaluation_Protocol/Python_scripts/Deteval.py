...
################ my edit for check the boost on different size of text instance ######################
global_size_table = []
gt_small = 0
gt_medium = 0
gt_large = 0
...
for i, input_id in enumerate(tqdm(allInputs)):
    if (input_id != '.DS_Store') and (input_id != 'Pascal_result.txt') and (
        ...
        local_size_table = np.zeros(len(detections))
        
        flag = True
        for gt_id, gt in enumerate(groundtruths):
            gt_x = list(map(int, np.squeeze(gt[1])))
            gt_y = list(map(int, np.squeeze(gt[3])))
            pts = np.array(np.stack([gt_x, gt_y])).T
            pts_area = cv2.contourArea(pts)
            if pts_area < 1000:
                gt_small += 1
            elif pts_area < 10000:
                gt_medium += 1
            ...
            if len(detections) > 0:
                for det_id, detection in enumerate(detections):
                    ...
                    #### get det size
                    if flag:
                        pts = np.array(detection).reshape(-1,2)
                        pts_area = cv2.contourArea(pts)
                        if pts_area < 1000:
                            local_size_table[det_id] = 1
                        elif pts_area < 10000:
                            local_size_table[det_id] = 2
                        else:
                            local_size_table[det_id] = 3
                    ################
        ...
        global_size_table.append(local_size_table)
...
for idx in range(len(global_sigma)):
    ...
    local_size_table = global_size_table[idx]
    match_table = local_size_table[det_flag[0]==1]
...
print(str_write)
print("small/middle/large/sum : {}/{}/{}/{}".format(small_match, middle_match, large_match, sumofmatch))
print("gt : small/middle/large : {}/{}/{}".format(gt_small, gt_medium, gt_large))
print("gt% : small/middle/large : {}/{}/{}".format(small_match / gt_small, middle_match / gt_medium, large_match / gt_large))
print('Done.\n')