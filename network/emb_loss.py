...
#     def __init__(self):
#         self.background_loss = nn.CosineEmbeddingLoss(margin = 0)
#         self.inst_loss = nn.CosineEmbeddingLoss(margin = 0.3)


#     def forward(self, embed, instance):
#         b,c,_,_ = embed.shape
#         for i in range(b):
#             emb = embed[i]
#             inst = instance[i]
...
#     def single_cos_loss(self, emb, inst):
#         c = emb.shape[0]
#         emb = emb.reshape(c,-1)
#         inst = inst.reshape(-1)

#         back_mask = embed[inst == 0]
#         back_label = torch.zeros(back_mask.shape)
#         inst_mask = embed[inst > 0]
#         keys = torch.unique(inst, sorted=True)
#         for k in keys:
#             key_mask = inst == k
#             withback = embed[back_mask or key_mask]
#             withbacklabel = 
